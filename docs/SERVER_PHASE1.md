...
Панель хранит желаемые настройки с номером ревизии. Агент получает их в ответе
на очередной heartbeat, применяет к локальному `config.json` и в следующем
heartbeat подтверждает ревизию. Доступны способ ввода, суффикс, задержки,
подавление повторов, фильтр, замена GS1 и белый список 2D-форматов.

Команда «Отвязать телефоны» также передаётся по исходящему heartbeat: агент
удаляет локальные сессии телефонов и выпускает новый QR-код. Входящие порты на
рабочем ПК для fleet-управления не открываются. При недоступности центрального
сервера локальное сканирование продолжает работать с последними настройками.
